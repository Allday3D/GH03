...
void loop() {
  ...
  if (pos) {
    ...
    if (Ethernet::buffer[59] == 97) { //97 = a
      Serial.println("[Switch a]");
      if (output01State == "off") {
        output01State = " on";
        digitalWrite(output01, HIGH);
        ether.httpServerReply(homePageaOn());
      }
      else if (output01State == " on") {
        output01State = "off";
        digitalWrite(output01, LOW);
        ether.httpServerReply(homePageaOff());
      }
    }

    if (Ethernet::buffer[59] == 98) { //98 = b
      Serial.println("[Switch b]");
      if (output02State == "off") {
        output02State = " on";
        digitalWrite(output02, HIGH);
        ether.httpServerReply(homePagebOn());
      }
      else if (output02State == " on") {
        output02State = "off";
        digitalWrite(output02, LOW);
        ether.httpServerReply(homePagebOff());
      }
    }

    if (Ethernet::buffer[59] == 99) { //99 = c
      Serial.println("[Switch c]");
      if (output03State == "off") {
        output03State = " on";
        digitalWrite(output03, HIGH);
        ether.httpServerReply(homePagecOn());
      }
      else if (output03State == " on") {
        output03State = "off";
        digitalWrite(output03, LOW);
        ether.httpServerReply(homePagecOff());
      }
    }

    if (Ethernet::buffer[59] == 100) { //100 = d
      Serial.println("[Switch d]");
      if (output04State == "off") {
        output04State = " on";
        digitalWrite(output04, HIGH);
        ether.httpServerReply(homePagedOn());
      }
      else if (output04State == " on") {
        output04State = "off";
        digitalWrite(output04, LOW);
        ether.httpServerReply(homePagedOff());
      }
    }
    show_info();
    printHours();
